...
describe ApplicationController do
  ...
  describe "Homepage" do
    it 'loads the homepage' do
      ...
      expect(last_response.body).to include("Welcome to Acoustic Karma")
    end
  end
  ...
  describe "logout" do
    ...
    it 'redirects a user to the login route if a user tries to access /posts route if user not logged in' do
      get '/posts'
      expect(last_response.location).to include("/login")
      expect(last_response.status).to eq(302)
    end

    it 'loads /posts if user is logged in' do
      user = User.create(:username => "becky567", :email => "starz@aol.com", :password => "kittens")
